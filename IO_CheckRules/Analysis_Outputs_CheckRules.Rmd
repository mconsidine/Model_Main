...
# Scenario 3 
Rxxhs
Single cell 20 - 60, with new entrants, stochastic return, starting off fully funded. 
...
results_all %>% filter(sim %in% 3, year<=80, runname %in% paste0(prefix, seq(20,60,10), "hs")) %>% select(runname, year, FR) %>% 
  ggplot(aes(x = year, y = FR)) + geom_line() + geom_point() + facet_grid(. ~ runname) + labs(title = "WITHOUT restriction") + 
  coord_cartesian(ylim = c(0,300))

results_all %>% filter(sim %in% 3, year<=80, runname %in% paste0(prefix, seq(20,60,10), "hsr")) %>% select(runname, year, FR) %>% 
  ggplot(aes(x = year, y = FR)) + geom_line() + geom_point() + facet_grid(. ~ runname) + labs(title = "WITH restriction") +
  coord_cartesian(ylim = c(0,300))
